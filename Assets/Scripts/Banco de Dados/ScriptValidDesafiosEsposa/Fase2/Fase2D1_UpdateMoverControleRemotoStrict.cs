...
public class Fase2D1_UpdateMoverControleRemotoStrict : MonoBehaviour
{
    [Header("Alvo da interação")]
    public FurnitureInteractable furniture;

    private const int ID_SOFA = 201;
    private const int ID_TV = 202;
    ...
    void Awake()
    {
        ...
        furniture.validator = new Func<string, int, DatabaseManager, bool>(ValidateUpdateMoverControleRemoto_AnyOrderWhere);
    }

    private bool ValidateUpdateMoverControleRemoto_AnyOrderWhere(string sql, int _, DatabaseManager __)
    {
        ...
        var m = Regex.Match(
            cleaned,
            @"^\s*update\s+itens\s+set\s+idmovel\s*=\s*(?:202|'202'|""202"")\s+where\s+(.*?)\s*$",
        ...
        foreach (var c in conds)
        {
            ...
            if (Regex.IsMatch(c, @"^nomeitem\s*=\s*(?:'Controle remoto'|""Controle remoto"")$", RegexOptions.IgnoreCase))
            {
                hasNomeControle = true;
                continue;
            }
        ...
        return hasIdMovel201 && hasNomeControle;
    }
